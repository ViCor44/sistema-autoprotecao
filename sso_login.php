<?php
/**
 * SSO Login Handler — Sistema de Autoprotecção
 * Fazer upload para: /sistema-autoprotecao/sso_login.php no servidor
 */

define('SSO_SYSTEM_KEY', 'autoprotecao');
...
// ── Configurar sessão igual ao Application::startSession() ───
...
try {
    ...
    $stmt = $pdoLocal->prepare(
        "SELECT * FROM {$tbl} WHERE {$idField} = ? LIMIT 1"
    );
    $stmt->execute([$userId]);
    $user = $stmt->fetch();
    ...
    // ── 6. Iniciar sessão (variáveis que o Sistema de Autoprotecção espera) ──
